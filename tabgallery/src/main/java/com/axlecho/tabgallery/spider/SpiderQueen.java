...
import com.axlecho.tabgallery.tools.ChromeRequestBuilder;
...
public final class SpiderQueen implements Runnable {
    ...
    private SpiderQueen(Context context, @NonNull ImageTabInfo galleryInfo) {
        mHttpClient = getOkHttpClient(context);
        mGalleryInfo = galleryInfo;
        mSpiderDen = new SpiderDen(mGalleryInfo);

        mWorkerMaxCount = MathUtils.clamp(3, 1, 10);
        mPreloadNumber = MathUtils.clamp(5, 0, 100);
    ...
    public static SpiderQueen obtainSpiderQueen(@NonNull Context context,
                                                @NonNull ImageTabInfo galleryInfo, @Mode int mode) {
        OSUtils.checkMainLoop();
        SpiderQueen queen = sQueenMap.get(galleryInfo.gid);
        if (queen == null) {
            queen = new SpiderQueen(context, galleryInfo);
    ...
    private void runInternal() {
        ...
        // Setup page state
        synchronized (mPageStateLock) {
            mPageStateArray = new int[mGalleryInfo.imgs.length];
        }
        // Notify get pages
        notifyGetPages(mGalleryInfo.imgs.length);
    ...
    private class SpiderWorker implements Runnable {
        ...
        private String getImageUrl(int index) throws Exception {
            if (DEBUG_LOG) {
        ...
        // false for stop
        private boolean downloadImage(int index, boolean force) {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                try {
                    ...
                    Call call = mHttpClient.newCall(new ChromeRequestBuilder(imageUrl).build());
                    ...
                    // Get out put pipe
                    pipe = mSpiderDen.openOutputStreamPipe(index, null);
                    if (null == pipe) {
                        // Can't get pipe
        ...
        private boolean runInternal() {
            ...
            // Get image url
            return downloadImage(index, force);
        }
